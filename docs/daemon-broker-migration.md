...
6. **After Phase 4 lands — round 1 done.** Three findings, all applied (see Phase 4 table above):
   ...
   `handle_find_session_by_id`'s lockless walk confirmed correct (WAL + busy_timeout + per-repo write lock). Open for round 2: the dead-weight `DetachSession` RPC — keep as a forward-compat seam for multi-window or elide and re-add later?

   **Post-merge follow-up applied (2026-04-24):** the tracked "Tauri's `stop_session` still bypasses the daemon's `StopSession` RPC" follow-up was resolved in commit `922b2e8`. Live-user reproduction exposed the full bite: the direct-pipe stop wrote `StopGraceful` as the first frame (SHA-43 silent discard), `tokio::time::timeout` swallowed the 5s hang, the Tauri backend wrote DB `stopped` while the supervisor kept running, and the daemon's live registry still pointed at a live PID — so a follow-up `RemoveSession` hit the Phase 4 two-stage guard with "still live — stop it first". The fix rewrote `handle_stop` to delegate to `stop_session_and_wait` (Resume preamble + drain + force-kill fallback), drop the live-registry entry, write DB `stopped` as a backstop, and broadcast `SessionsChanged`. Tauri's `stop_session` collapsed to a 3-line translator over the new `daemon_ipc::stop_session` helper. Validated via Playwright: fresh session → Stop → session disappears cleanly, no orphan supervisors, no "still live" error. No new integration test landed yet (the harness doesn't spawn real supervisors so this would need the fake-supervisor pattern from Phase 1 extended to cover the stop-lifecycle path — low priority given the live-repro confirmation).
